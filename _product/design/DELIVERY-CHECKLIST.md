...
#### 3.5 已知的必然冲突与解法：`tests\Galbox.Acceptance\Program.cs`
...
**7 条待合并的工作线都会在同一个位置（数组末尾、闭合大括号之前）追加自己的检查。**
这意味着 **`Program.cs` 在每一次合并时都会冲突** —— 不是因为改动互相矛盾，而是因为它们在文件里的位置相邻。

**解法（每次都一样，机械操作）**：
1. 冲突时不要选"ours"或"theirs"，**手工取并集**
2. 保持既有编号的原有顺序（A0…A9、A10、A11…A19、A30…、A40…、A50…、A60…、A70…）
3. 只保留一个闭合 `};`
4. 合并后**核对注册项数**是否等于各分支之和，并用 grep 确认没有重复编号：
   ```
   Select-String -Path tests\Galbox.Acceptance\Program.cs -Pattern 'new\s+(A\d+)\w*Check' -AllMatches
   ```
5. **合并后必须跑一次验收**——注册数组写错会让检查静默不执行，而这种失败不会自我暴露

#### 3.6 尚未验证：全链路端到端检查

目标原文要求"**真实游戏跑通全链路**"。当前 A0–A19 是**按功能逐项验证**，还没有一项检查走完**完整用户旅程**：

> 扫描目录 → 添加游戏 → 刮削元数据 → 定位存档 → 解析存档 → 展示剧情节点 → 备份 → 恢复

**待各线合并后补一项端到端检查**（编号接在最后），在真实游戏 `D:\GAME\Dreamin'_Her` 上跑通整条链并断言每一步的产物。
这是"完成度"最有说服力的单一证据。

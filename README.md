...
>
> ⚠️ **运行验收程序会真的启动 GUI**：A9 / A18 / A19 会开出真实窗口，A50 会快速切换导航 200 次。
> 不想被弹窗打断时，请先读「五、运行验收程序」里的提醒。
...
> ⚠️ **它会开真实窗口。** 43 项检查里有四项会启动 `Galbox.App.exe`：A9（带刮削缓存启动）、
> A18（查窗口标题）、A19（逐个导航页加载）、A50（**快速切换导航最多 200 次**）。
> 它们会短暂抢走前台焦点；A50 在低配机器或与其它验收进程并行时会跑满几十秒。
> 只想验证业务逻辑时，请先看完本节再决定什么时候跑。
...
本文件写作时的实测结果（Windows + .NET SDK 8.0.425，游戏目录 `D:\GAME\Dreamin'_Her` 存在）：
...
失败的是 **A50**（快速导航压力检查）：它自己的日志显示应用进程在第 63 次切换时消失，
退出码 `0x00000000`（正常退出，不是崩溃）。当时同一台机器上**还有另外两条工作线的验收程序
在并行运行**（它们同样会启动并关闭 `Galbox.App.exe`），而 A19 的注释恰好记录了这种互相干扰。
所以这一次的 A50 不能当作产品缺陷读，也**不能当作通过读**——它需要在不并行跑验收的机器上复现。

除 A50 外的 42 项全 PASS，包括本文表格里引用的 A10、A30–A32、A40–A42、A70–A74、A90–A92。
`--verbose` 会打印每项检查期间的服务日志。上面这段数字对应的**完整原始输出**留档在
`_verify/acceptance-runs/docsfix-accuracy-fix-42of43-A50-interference.txt`（含 A50 失败时它自己打印的
逐行证据）。
...
├── tests/
│   ├── Galbox.Acceptance/    # 无界面验收程序（43 项，本项目唯一可信的自动化验证）
...
### 8.5 补丁中心：本地安装链路已接通，在线源只有服务层（界面接线进行中）
...
**在线补丁源（moyu.moe）现在有服务层，但界面还没接上：**

* 服务层已实现并验收：只允许访问官方 NextMoe 的 `/v2/moyu` 面，密钥由用户自带（`nmk_`，
...
* **界面接线还没完成**：当前补丁中心里仍然写着「在线补丁源：未实现」
  （文案在 `PatchCenterViewModel.Patches.cs:275-276`，展示在 `PatchCenterPage.xaml:548-554`），
  没有记录时的空状态也仍然说"尚未配置在线补丁源"（同文件 520 行）。**另一条工作线正在接这一步**，
  接完之后这里的说法会随之更新——在那之前，请不要把在线源当成能点出来的功能。
...
* A50（快速导航压力检查）最近一次实测未通过，原因与当前状态见「五、运行验收程序」。
